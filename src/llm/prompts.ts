...
export function keyPointsPrompt(question: ParsedQuestion): { system: string; user: string } {
  ...
  return {
    system: [
      ...
      '   - Vitals: ONLY the abnormal ones, each prefixed with conventional shorthand so the number is interpretable at a glance — T (temp in F), BP (as "X over Y"), HR, RR, SpO2. Group them on one bullet: "- T 101, BP 100 over 60, HR 120, RR 28, SpO2 88".',
      '   - Exam: positive findings the stem describes + any specifically denied finding ("no murmur", "lungs clear", "no rash")',
      '   - Labs / imaging / studies: ONLY abnormal values, or normals the stem specifically calls attention to. Prefix every lab value with its test abbreviation (Hb, WBC, Plt, Na, K, Cr, BUN, Gluc, AST, ALT, TSH, etc.) so the bare number means something: "- Hb 8, WBC 18, Cr 2.1".',
      ...
      '- NO unit suffixes — never write "mm Hg", "mg/dL", "bpm", "mL", "kg", "%". The shorthand prefix (T, BP, HR, Hb, Cr, …) tells the reader what the number is. The ONLY exception: trailing "F" after a temperature is allowed ("T 101 F") since temperature scale is ambiguous otherwise.',
      '- All temperatures are already in Fahrenheit. Never write Celsius.',
      '- Render BP "80/50" as "80 over 50". Other ratios (I/E, A/G) keep the slash.',
      '',
