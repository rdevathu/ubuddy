...
export function intensePrompt(question: ParsedQuestion): { system: string; user: string } {
  ...
  return {
    system: [
      ...
      'Your output is fed directly to a TTS engine and read aloud while the student reads choices on screen.',
      '',
      '═══ OUTPUT FORMAT (INVIOLABLE) ═══',
      'ONE single paragraph of plain prose. 2-3 short sentences. Roughly 40 spoken seconds maximum.',
      ...
      '- NO units anywhere — never say "mm Hg", "milligrams per deciliter", "Celsius", "per minute". Just the number.',
      '- All temperatures are already in Fahrenheit. Never speak Celsius.',
      '- Render "80/50" as "80 over 50". Render fractions in spoken form.',
